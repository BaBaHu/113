...
//设置扑克
bool CHeapCard::SetHeapCardInfo(WORD wMinusHeadCount, WORD wMinusLastCount,BYTE cbTailCount)
{
	...
	m_cbTailCount=cbTailCount;

	if(m_cbTailCount>0)
	{
		m_wMinusHeadCount=0;
		m_wMinusLastCount=0;
		SetCardEmpty(m_wFullCount-m_cbTailCount-1,wMinusHeadCount+wMinusLastCount);
	}
...
void CHeapCard::SetCardEmpty( WORD wIndex, BYTE cbEmptyCount )
{
	...
	{
		m_cbCardData[i] = 255;
		if(i == wIndex+1-cbEmptyCount && (i%2)!=0 && i>0 && m_cbCardData[i-1]!=255)
		{
			m_cbCardData[i]=m_cbCardData[i-1];
			m_cbCardData[i-1]=255;
		}
	}
	return;
}
...
//获取出牌位置
CPoint CHeapCard::GetDispatchCardPos( bool bTail ,WORD wMinusHeadCount, WORD wMinusLastCount,BYTE cbTailCount)
{
	//设置变量
	m_wMinusHeadCount=wMinusHeadCount;
	m_wMinusLastCount=wMinusLastCount;
	...
	case Direction_South:	//南
		{
			...
			if( !bTail )
			{
				...
				if(m_cbTailCount>0)nXExcusion = (m_wMinusLastCount+m_cbTailCount+m_wMinusHeadCount)/2+1;
				ptHead.x -= nXExcusion*g_CardResource.m_ImageHeapDoubleH.GetWidth();
				if( m_wMinusHeadCount%2 != 0 || m_cbTailCount>0) ptHead.y += 9;
			}
			else
			{
				nXExcusion = (m_wFullCount-m_wMinusLastCount+1)/2;
				if(m_cbTailCount>0)nXExcusion = m_cbTailCount/2+m_cbTailCount%2;
				ptHead.x -= nXExcusion*g_CardResource.m_ImageHeapDoubleH.GetWidth();
				if( m_wMinusLastCount%2 != 0 || m_cbTailCount%2 != 0) ptHead.y += 9;
			}
		}
		break;
	case Direction_North:	//北
		{
			...
			if( !bTail )
			{
				...
				if(m_cbTailCount>0)nXExcusion = (m_wMinusLastCount+m_cbTailCount+m_wMinusHeadCount)/2;
				ptHead.x += nXExcusion*g_CardResource.m_ImageHeapDoubleH.GetWidth();
				if( m_wMinusHeadCount%2 != 0  || m_cbTailCount>0) ptHead.y += 9;
			}
			else
			{
				...
				if(m_cbTailCount>0)nXExcusion = m_cbTailCount/2-((m_cbTailCount%2)?0:1);
				ptHead.x += nXExcusion*g_CardResource.m_ImageHeapDoubleH.GetWidth();
				if( m_wMinusLastCount%2 != 0 || m_cbTailCount%2 != 0) ptHead.y += 9;
			}
		}
...
bool CHeapCard::DeductionHeapCard( bool bTail  ,BYTE &wMinusHeadCount, BYTE &wMinusLastCount,BYTE &cbTailCount,bool bChangeTail[])
{
	ResetCardData();
	m_cbTailCount=cbTailCount;
	m_wMinusHeadCount=wMinusHeadCount;
	...
	if( !bTail )
	{
		if( m_wMinusHeadCount+m_wMinusLastCount+m_cbTailCount >= m_wFullCount )
		{
			if(cbTailCount>0)
			{
				cbTailCount=0;
				m_cbTailCount=0;
				wMinusLastCount+=wMinusHeadCount;
				wMinusHeadCount=0;
				m_wMinusHeadCount=wMinusHeadCount;
				m_wMinusLastCount=wMinusLastCount;
			}
	...
	else 
	{
		...
		if(m_cbTailCount>0)
		{
			...
			if(m_cbTailCount==0)
			{
				wMinusHeadCount+=wMinusLastCount+1;
				wMinusLastCount=0;
				m_wMinusHeadCount=wMinusHeadCount;
				m_wMinusLastCount=wMinusLastCount;
				bChangeTail[0]=((m_wMinusHeadCount+m_wMinusLastCount)<HEAP_FULL_COUNT);
			}
		}
	...
	cbTailCount=m_cbTailCount;
	wMinusHeadCount = (BYTE)m_wMinusHeadCount;
	wMinusLastCount = (BYTE)m_wMinusLastCount;
	if(m_cbTailCount>0)
	{
		m_wMinusHeadCount=0;
		m_wMinusLastCount=0;
...
CUserCard::CUserCard()
{
	...
	//控制变量
	m_ControlPoint.SetPoint(0,0);
	m_CardDirection=Direction_North;
...
//玩家吃牌
bool CCardControl::OnEventUserAction( const tagSelectCardInfo SelectInfo[], BYTE cbInfoCount )
{
	//初始化
